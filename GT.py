...
# Crear curva granulométrica con el estilo solicitado
st.subheader("CURVA DE DISTRIBUCIÓN GRANULOMÉTRICA")
fig, ax = plt.subplots(figsize=(10, 6))

# Configuración de la gráfica - Estilo similar a la imagen
ax.set_xscale('log')
...
# INVERTIR EJE X (de mayor a menor)
ax.set_xlim(100, 0.01)  # Límites invertidos

# Formateador para valores decimales en eje X
formatter = ScalarFormatter()
formatter.set_scientific(False)
ax.xaxis.set_major_formatter(formatter)

# Configurar ticks del eje X para que coincidan con la imagen
...
# Graficar curva con estilo similar (línea continua azul)
ax.plot(curve_df['Abertura (mm)'], curve_df['Pasa (%)'], 
...
try:
    ...
    # Añadir líneas horizontales y verticales con estilo similar a la imagen
    for pct, diam, color, label in zip(
        ...
        ax.plot(diam, pct, 'o', markersize=8, color=color, fillstyle='none', markeredgewidth=1.5, zorder=2)
        ax.annotate(f'{label} = {diam:.3f} mm', 
                   xy=(diam, pct), 
                   xytext=(diam*1.2, pct+3),
                   fontsize=10,
